...
class Status:
    """
    Class responsible for the status commands.
    """
    ...
    async def do_command(self, update: Update, context: CallbackContext, type: str) -> None:
        ...
        try:
            ...
            await self.function.send_message(escape_markdown(f"Retrieving data from API for {type} monitor(s) failed, see the logs for more information.", version=2), context)
    ...
    async def create_status_message(self, json: Dict[str, Any], type: str) -> str:
        ...
        def get_section_value(json_data: Dict[str, Any], section: str, key: str, default=0) -> Any:
        ...
        # APT Updates
        if type in ["all", "apt"]:
            total_updates = get_section_value(
        ...
        if type in ["all", "processes"]:
            processes = get_section_value(
                json, "process_status", "processes", {})
